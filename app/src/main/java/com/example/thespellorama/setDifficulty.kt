...
class setDifficulty : AppCompatActivity() {
    ...
    fun _getWord(setting: String): String {
        ...
        // of wordToGuess
        if (setting == "k") {
            return _openFile("app/src/main/res/gradek")
        }
        else if (setting == "1") {
            return _openFile("app/src/main/res/grade1")
        }
        else if (setting == "2") {
            return _openFile("app/src/main/res/grade2")
        }
        else if (setting == "3") {
            return _openFile("app/src/main/res/grade3")
        }
        else if (setting == "4") {
            return _openFile("app/src/main/res/grade4")
        }
        else if (setting == "5") {
            return _openFile("app/src/main/res/grade5")
        }
        return "This is broken"
    }

    fun _openFile(file : String): String {
        // Get word from a file, store as a list/array and gives difficulty word back to be stored
        println("This is firing")
        var fileName = File(file)
        val words : List<String> = fileName.readLines()
